...
import { createBlockageCode, generateAccessToken, generateRefreshToken } from "../helpers/test.helpers";
...
it("updates on cascade a product blockage type if a product was using the blockage type", async () => {
  ...
  const product = await request(app)
    ...
    .send({
      ...
      "price": 23.70,
      "product_blockage_name": `${productBlockage.body.name}`
    })
    .expect(201);

  const updatedBlockage = await request(app)
    .patch(`/api/product/blockage/${productBlockage.body.id}`)
    .set("authorization", `${await generateAccessToken()}`)
    .set("x-refresh-token", `${await generateRefreshToken()}`)
    .send({
      "name": "YYY"
